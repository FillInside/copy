...
def _read_and_decode(filename_queue, image_dim=48, distort=False, split='train'):
    ...
    if image_dim < SMALLNORB_SIZE:
        ...
        raise ValueError('Image dim must be <= {}, got {}'.format(SMALLNORB_SIZE, image_dim))
    ...
    if distort and split == 'train':
        ...
        image = tf.random_crop(image, tf.stack([distorted_dim, distorted_dim, depth]))
        image_dim = distorted_dim
    elif distort:
        image = tf.image.resize_image_with_crop_or_pad(image, distorted_dim, distorted_dim)
        image_dim = distorted_dim
...
def inputs(data_dir, batch_size, split, height=48, distort=True,
           batch_capacity=5000,):
    """Reads input data.
    Args:
