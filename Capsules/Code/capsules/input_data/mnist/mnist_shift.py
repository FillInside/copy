...
def sharded_writers(size, max_shard, num_images, file_prefix):
    ...
    Args:
      ...
      file_prefix: String, prefix of the name of the resultant sharded tfrecord file.
    Returns:
...
def shift_write_multi_mnist(input_dataset, file_prefix, shift, pad, max_shard,
    ...
    Args:
      ...
      pad: Integer, the number of pixels to be padded.
      max_shard: Integer, maximum number of examples in each shard.
      num_pairs: Integer, number of pairs of images generated for each input image.
    """
...
def read_byte_data(data_dir, split):
    ...
    The format of the binary files are defined at: http://yann.lecun.com/exdb/mnist/
    In summary: header size for image files is 4 * 4 bytes and for label file is 2 * 4 bytes.
    Args:
      data_dir: String, the directory containing the dataset files.
      split: String, the dataset split to process. It can be one of train, test, valid_train, valid_test.
    Returns:
...
def main(_):
    ...
    filename = os.path.join(FLAGS.data_dir, file_format.format(FLAGS.split, FLAGS.shift))
    if FLAGS.multi_targets:
