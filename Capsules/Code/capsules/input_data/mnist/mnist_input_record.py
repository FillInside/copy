...
def _read_and_decode(filename_queue, image_dim=28, distort=False, split='train'):
...
def _multi_read_and_decode(filename_queue):
    ...
    # Convert from [0, 255] -> [-0.5, 0.5] floats.
    features['image_raw_1'] = tf.cast(features['image_raw_1'], tf.float32) * (1. / 255)
    features['image_raw_2'] = tf.cast(features['image_raw_2'], tf.float32) * (1. / 255)
    features['merged_raw'] = tf.cast(features['merged_raw'], tf.float32) * (1. / 255)
    ...
    res_features['labels'] = tf.one_hot(features['label_1'], 10) + tf.one_hot(features['label_2'], 10)
...
def inputs(data_dir, batch_size, split, num_targets, height=28,
           distort=False, batch_capacity=5000, validate=False,):
    """Reads input data.
    Args:
      ...
      validate: If set use training-validation for training and validation for test.
    Returns:
