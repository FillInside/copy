    ...
    for i in range(num_gpus):
        with tf.device('/gpu:%d' % i):
            if dataset == 'mnist':
                ...
                features.append(norb_input_record.inputs(data_dir=data_dir, batch_size=batch_size, split=split,))
            elif dataset == 'cifar10':
                data_dir = os.path.join(data_dir, 'cifar-10-batches-bin')
                features.append(cifar10_input.inputs(split=split, data_dir=data_dir, batch_size=batch_size))
           else:
               raise ValueError('Unexpected dataset {!r}, must be mnist, norb, or cifar10.'.format(dataset))
    return features
...
def train_experiment(session, result, writer, last_step, max_steps, 
                     saver, summary_dir, save_step):
    ...
    for i in range(last_step, max_steps):
        ...
        if (i + 1) % save_step == 0:
            saver.save(session, os.path.join(summary_dir, 'model.ckpt'), global_step=i + 1)
...
def eval_experiment(session, result, writer, last_step, max_steps, 
                    **kwargs):
    ...
    for _ in range(max_steps):
        summary_i, correct, almost = session.run([result.summary, result.correct, result.almost])
    ...
    print('Total wrong predictions: {}, wrong percent: {}%'.format(total_false, total_false / max_steps))
    tf.logging.info('Total wrong predictions: {}, wrong percent: {}%'.format(total_false, total_false / max_steps))
    writer.add_summary(summary, last_step)


def run_experiment(loader, load_dir, writer, experiment, result, 
                   max_steps, save_step=0):
...
def train(hparams, summary_dir, num_gpus, model_type, max_steps, 
          save_step, data_dir, num_targets, dataset, validate):
...
def evaluate(hparams, summary_dir, num_gpus, model_type, eval_size, 
             data_dir, num_targets, dataset, validate, checkpoint=None):
...
def get_placeholder_data(num_steps, batch_size, features, session):
    ...
    features['labels'] = tf.placeholder(tf.float32, shape=(batch_size, num_classes))
...
def evaluate_ensemble(hparams, model_type, eval_size, data_dir, num_targets,
    ...
    with tf.Graph().as_default():
        ...
        data, targets = get_placeholder_data(num_steps, batch_size, features, session)
        logits = infer_ensemble_logits(features,  
                                       model, 
                                       checkpoints, 
                                       session,
                                       num_steps, 
